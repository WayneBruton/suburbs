        ...
        if ($(this).prop('name') == 'noActiveCategories') {
            ...
            $.get(url).done((response) => {
                ...
                let profileToShow = `<div id="p_display"
                                        ...
                                        style=" border: 3px solid ${viewColor};border-radius: 7px;">
                                        <br>
                                        <p>${profileDescription}</p>
                                        <button id="${response[i].id}" class="contactBtns" style="background-color: ${viewColor};color: white;margin:5px 5px; border: 1px solid ${viewColor}; border-radius: 7px; width: 100%"><a href="/showProfile/${response[i].id}" style="color: white; text-decoration: none">View Profile</a></button></div>` 
